...
/*  print_mpi                                                               */
/* ************************************************************************ */
static void print_mpi(const char* function, int rank, double start, double end, int blocking) {
	if (blocking) {
		printf("[Rank %i] %s: called at: %.6f  ended at: %.6f, duration: %.6f \n", rank, function, start - startTime, end - startTime, end - start);
	} else {
		printf("[Rank %i] %s: called at %.6f \n", rank, function, start - startTime);
	}
}
...
int MPI_Recv(void *buf, int count, MPI_Datatype datatype, int source, int tag, MPI_Comm comm, MPI_Status *status) {
	...
	print_mpi("MPI_Recv", rank, start, end, 1);
	return result;
}
...
int MPI_Bcast(void *buffer, int count, MPI_Datatype datatype, int root, MPI_Comm comm) {
	...
	double end = MPI_Wtime();
	print_mpi("MPI_Bcast", rank, start, end, 1);
	return result;
}
/* ************************************************************************ */
/*  MPI_Issend                                                              */
...
int MPI_Issend(const void *buf, int count, MPI_Datatype datatype, int dest, int tag, MPI_Comm comm, MPI_Request *request) {
	...
	PMPI_Comm_rank(comm, &rank);
	double start = MPI_Wtime();
	int result = PMPI_Issend(buf, count, datatype, dest, tag, comm, request);
	print_mpi("MPI_Issend", rank, start, 0, 0);
	return result;
}
/* ************************************************************************ */
/*  MPI_Waitall                                                             */
/* ************************************************************************ */
int MPI_Waitall(int count, MPI_Request arrayR[], MPI_Status arrayS[]) {
	int rank;
	PMPI_Comm_rank(MPI_COMM_WORLD, &rank);
	double start = MPI_Wtime();
	int result = PMPI_Waitall(count, arrayR, arrayS);
	double end = MPI_Wtime();
	print_mpi("MPI_Waitall", rank, start, end, 1);
	return result;
}

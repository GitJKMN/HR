...
void allocateMatrices(void) {
  ...
  Matrix[0] =
      (double **)calloc((N + 1), sizeof(double *)); /* allocate memory */
  if (Matrix[0] == 0) {
    errorQuit();
  } /* quit if error   */
...
void freeMatrices(void) {
  ...
  if (Matrix[0] != 0)
    free(Matrix[0]);
  free(M);
  free(Matrix); //Referenzierung über pointer ist nur möglich den pointer auf etwas zeigt
}
...
void calculate(void) {
  ...
  while (term_iteration > 0) {
    ...
    {                         /*                   */
      ...
      {
        star = -Matrix[m2][i - 1][j] - Matrix[m2][i][j - 1] +   //Indexe vertauscht
